...
use winit_integration::{RepaintSignal, UserEvent};
...
impl<'app> WgpuWinitApp<'app> {
    ...
    fn init_run_state(
        ...
        // TODO: Re-enable accesskit when winit 0.31 compatibility is sorted out
        // #[cfg(feature = "accesskit")]
        // {
...
fn create_window(
...
) -> Result<(Arc<dyn Window>, ViewportBuilder), winit::error::RequestError> {
    profiling::function_scope!();
    ...
    let window: Arc<dyn Window> = Arc::from(egui_winit::create_window(
        egui_ctx,
        event_loop,
        &viewport_builder,
    )?);
