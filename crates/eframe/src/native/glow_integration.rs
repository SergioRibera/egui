...
use super::{
    ...
    winit_integration::{EventResult, RepaintSignal, UserEvent, WinitApp, create_egui_context},
};
...
pub struct GlowWinitApp<'app> {
    repaint_signal: Arc<egui::mutex::Mutex<RepaintSignal>>,
    event_receiver: std::sync::mpsc::Receiver<UserEvent>,
    app_name: String,
...
impl<'app> GlowWinitApp<'app> {
    pub fn new(
        ...
        let repaint_signal = RepaintSignal::new(event_loop.create_proxy(), sender);
        Self {
            repaint_signal: Arc::new(egui::mutex::Mutex::new(repaint_signal)),
    ...
    fn init_run_state(
...

        {
            ...
            integration
                ...
                .set_request_repaint_callback(move |info| {
                    ...
                    repaint_signal.lock().send(UserEvent::RequestRepaint {
        ...
        // TODO: accesskit needs to be updated for winit 0.31 (no send_event)
        // #[cfg(feature = "accesskit")]
        // {
        //     let event_loop_proxy = self.repaint_signal.lock().proxy.clone();
    ...
    fn on_window_event(
        ...
        if let Some(viewport_id) = viewport_id {
            if let Some(viewport) = glutin.viewports.get_mut(&viewport_id) {
                ...
                {
                    event_response = self.integration.on_window_event(window.as_ref(), egui_winit, event);
                }
            } else {
...
impl GlutinWindowContext {
    ...
    unsafe fn new(
        ...
        */
        ...
        if let Some(window) = &window {
            egui_winit::apply_viewport_builder_to_window(egui_ctx, window.as_ref(), &viewport_builder);
        }
    ...
    pub(crate) fn initialize_window(
        ...
        let window = if let Some(window) = &mut viewport.window {
            ...
            egui_winit::update_viewport_info(&mut viewport.info, &self.egui_ctx, window.as_ref(), true);
            viewport.window.insert(Arc::from(window))
        };
